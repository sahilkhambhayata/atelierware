...
const Menu = ({ sidebarToggle, mobileView }) => {
  ...
  // Extract allowed ModuleCodes and PageCodes from rolePermission
  const allowedModuleCodes = rolePermission?.map((module) => module.ModuleCode);
  const allowedPageCodes = rolePermission?.flatMap((module) =>
    module.PAGE.map((page) => page.PageCode)
  );

  // Filter menu items based on allowedModuleCodes
  const filteredMenu = menu?.filter((item) =>
    allowedModuleCodes?.includes(item.id)
  );

  // Filter sub-menu items based on allowedPageCodes
  const filteredMenuWithSubMenu = filteredMenu
    ?.map((item) => {
      if (item.subMenu) {
        const filteredSubMenu = item.subMenu.filter((subItem) =>
          allowedPageCodes?.includes(subItem.pageId)
        );
  ...
  return (
    <ul className="nk-menu">
      {/* {filteredMenuWithSubMenu?.map((item) => { */}
      {menu?.map((item) => {
        return item.heading ? (
