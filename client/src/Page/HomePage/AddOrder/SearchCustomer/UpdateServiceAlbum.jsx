...
import { getupServiceListAsyncData } from "../../../../redux/actions/serviceAction";
import { loading } from "./../../../../redux/actions/catelogAction";

const UpdateServiceAlbum = ({ onImageClick, searchService }) => {
  const handleClick = (item) => {
    onImageClick(item);
  };

  const BU_ID = localStorage.getItem("BU_Id");
  const service = useSelector((state) => state.service?.upservice);
  ...
  useEffect(() => {
    dispatch(getupServiceListAsyncData(BU_ID, searchService));
  }, []);
  ...
  return (
    <div className="row m-0 w-100 ">
      {!serviceNoData.noupData ? (
        !serviceNoData.isLoader ? (
...
export default UpdateServiceAlbum;
